...
def restart_from_checkpoint(ckp_path, run_variables=None, **kwargs):
    ...
    if not os.path.isfile(ckp_path):
        print("Can not found checkpoint at {}".format(ckp_path))
        return
...
class MultiCropCondWrapper(nn.Module):
    """
    Perform forward pass separately on each resolution input.
    The inputs corresponding to a single resolution are clubbed and single
    forward is run on the same resolution inputs. Hence we do several
    ...
    """
    def __init__(self, backbone, head):
        super(MultiCropCondWrapper, self).__init__()
        # disable layers dedicated to ImageNet labels classification
        backbone.fc, backbone.head = nn.Identity(), nn.Identity()
        self.backbone = backbone
        self.head = head

    def forward(self, x, semantic_weight):
        # convert to list
        if not isinstance(x, list):
            x = [x]
        idx_crops = torch.cumsum(torch.unique_consecutive(
        ...
        start_idx = 0
        for end_idx in idx_crops:
            _out,_feats = self.backbone(torch.cat(x[start_idx: end_idx]), torch.cat(semantic_weight[start_idx: end_idx]))
            if start_idx == 0:
                output = _out
                outfeats = _feats
            else:
            ...
            start_idx = end_idx
        # Run the head forward on the concatenated features.
        return self.head(output), outfeats[-1]
