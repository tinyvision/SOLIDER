...
def get_args_parser():
    ...
    # Semantic Clustering
    parser.add_argument('--partnum', default=3, type=int, help="""Predefined part number for Semantic Clustering""")
    parser.add_argument('--parthead_nlayers', default=3, type=int, help="""Number of layers for Semantic Head""")
    parser.add_argument('--semantic_loss', default=1.0, type=float, help="""Semantic Weight""")
    parser.add_argument('--resume', default=False, type=utils.bool_flag, help="""Whether use resume. If True, please specify the init model.""")
    parser.add_argument('--init_model', default='checkpoint.pth', type=str, help="""Init model for resume""")
...
def train_dino(args):
    ...
    # build part classification
    part_classifier = DINOHead(embed_dim,args.partnum+1,nlayers=args.parthead_nlayers).cuda()
    part_classifier = nn.parallel.DistributedDataParallel(part_classifier, device_ids=[args.gpu], find_unused_parameters=True)
    part_loss = nn.CrossEntropyLoss(reduction='none')

    # multi-crop wrapper handles forward with inputs of different resolutions
    student = utils.MultiCropCondWrapper(student, DINOHead(
        embed_dim,
        args.out_dim,
        use_bn=args.use_bn_in_head,
        norm_last_layer=args.norm_last_layer,
    ))
    teacher = utils.MultiCropCondWrapper(
        teacher,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, part_classifier, part_loss,
...
def build_onehot_semantic_weight(dim,setVal=-1):
    if setVal == -1:
        w = torch.randint(2,(dim,1)).float()
    elif setVal == 1:
        w = torch.ones(dim,1)
    elif setVal == 0:
        w = torch.zeros(dim,1)
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, part_classifier, part_loss, data_loader,
    ...
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 200, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            # teacher forward, semantic weight = 1
            weight_unit = build_onehot_semantic_weight(args.batch_size_per_gpu,setVal=1)
            semantic_weight = [weight_unit,weight_unit]
            _, teacher_feats = teacher(images[:2],semantic_weight)  # only the 2 global views pass through the teacher

            # teacher forward, semantic weight = randint
            weight_unit = build_onehot_semantic_weight(args.batch_size_per_gpu,setVal=-1)
            semantic_weight = [weight_unit,weight_unit]
            ...
            if len(mask_idxs) != 0:
                k = random.randint(1,args.partnum)
            ...
            loss1 = 1. * dino_loss(student_output[nimages.shape[0]:], teacher_output, epoch)

            # sum all losses
            if len(mask_idxs) != 0:
                semantic_weight1 = semantic_weight[0]
                semantic_weight2 = torch.cat(semantic_weight[1:3])[torch.from_numpy(mask_idxs)]
                semantic_weight = torch.cat([semantic_weight1,semantic_weight2])[:,0]
                semantic_weight = semantic_weight.repeat_interleave(int(loss2.shape[0]/semantic_weight.shape[0])).flatten()
                loss2 = torch.mean(args.semantic_loss * semantic_weight * loss2)
            loss = loss1 + loss2
        ...
        metric_logger.update(fgnum=len(mask_idxs))
        if len(mask_idxs) != 0:
...
class DataAugmentationDINO(object):
    def __init__(self, size, crop_size, global_crops_scale, local_crops_scale, local_crops_number):
        ...
        self.global_transfo1 = transforms.Compose([
            transforms.RandomResizedCrop(size=size, scale=global_crops_scale, interpolation=Image.BICUBIC, ratio=ratio),
        ...
        self.local_transfo = transforms.Compose([
            transforms.RandomResizedCrop(size=crop_size, scale=local_crops_scale, interpolation=Image.BICUBIC, ratio=ratio),
            flip_and_color_jitter,
